...
class MLP:
    ...
    def fit(self, X, Y):
        self.epochs = 1
        pred = np.zeros((Y.shape[0],self.number_neurons[2]))
        ...
        while(True):
            ...
            for j in range(Y.shape[0]):            
                ...
                self.weights[1][:,:,2] = (self.alpha+1)*self.weights[1][:,:,1] - self.alpha*self.weights[1][:,:,0]

                self.weights[1][:,:,2] = self.weights[1][:,:,2] + self.eta*delta.reshape((delta.shape[0],1)).dot(np.transpose(Y_1.reshape((Y_1.shape[0],1)))) 
...
if __name__=='__main__':
    train = pd.read_csv('data/train/project_2.csv', decimal=',')
    trainX = np.array(train.drop(columns=['d1','d2','d3']).values)
    trainY = np.array(train.drop(columns=['x1','x2','x3','x4']).values)
    test = pd.read_csv('data/test/project_2.csv', decimal=',')
    testX = np.array(test.drop(columns=['d1','d2','d3']).values)
    testY = np.array(test.drop(columns=['x1','x2','x3','x4']).values)
    
    model = MLP([4, 15, 3], 0.1, 0.9, sensibility=0.000001)
    ...
    for i in range(testY.shape[0]):
        pred.append(np.argmax(model.predict(testX[i]))+1)
    print('EPOCAS - {}'.format(model.epochs))    
    print('Resultados - {}'.format(pred))
    plt.plot(model.error)
    plt.title('Mean Squared Error')
    plt.grid(True)
    plt.xlabel('Epochs')
    plt.ylabel('MSE')
    plt.savefig('plot2_project2.png')
