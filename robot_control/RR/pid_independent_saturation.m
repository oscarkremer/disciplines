...
for i=1:sizes(1)-2
    ...
    tau = k_i.*int_error + k_v.*(q_dot_des(:,i)-q_dot0) + k_p.*(q_des(:,i) - q0);
    int_error = int_error + (q_des(:,i) - q0)*delta_time;
    if abs(tau(1)) > limit_torque
      tau(1) = sign(tau(1))*limit_torque;
    end
    if abs(tau(2)) > limit_torque
      tau(2) = sign(tau(2))*limit_torque;
    end
